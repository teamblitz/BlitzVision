...
class VisualOutputProcessor:
    ...
    def display_frame(self, frame, cam_id):
        ...
        for tag_detection in self.tag_detections[cam_id]:
            ...
            cv2.putText(display_frame, str(self.rop.april_tag_corners[tag_id.decode()][0]), (int(corners[0][0]), int(corners[0][1]) + textyheight),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 128, 128), 2)
            cv2.putText(display_frame, str(self.rop.april_tag_corners[tag_id.decode()][1]), (int(corners[1][0]), int(corners[1][1]) + textyheight),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 128, 128), 2)
            cv2.putText(display_frame, str(self.rop.april_tag_corners[tag_id.decode()][2]), (int(corners[2][0]), int(corners[2][1]) + textyheight),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 128, 128), 2)
            cv2.putText(display_frame, str(self.rop.april_tag_corners[tag_id.decode()][3]), (int(corners[3][0]), int(corners[3][1]) + textyheight),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 128, 128), 2)
